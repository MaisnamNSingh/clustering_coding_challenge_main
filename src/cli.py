...
parser.add_argument('--scheduler', default='Adam3',
...
parser.add_argument('--batch_size', default=10, type=int,
                    help='train mini-batch size (default: 10)')
...
parser.add_argument('--crop_version', default=None, type=str,
                    help='the cropped version (default: None)')
...
parser.add_argument('--pseudo', default=None, type=str,
                    help='pseudo type, such as chexpert_pseudo')
parser.add_argument('--pseudo_ratio', default=1., type=float,
                    help='pseudo ratio selected for each epoch')
...
def main():
    ...
    model_out_dir = opj(RESULT_DIR, 'models',
                        args.out_dir, 'fold%d' % args.fold)
    log.write(
    ...
    train_split_file = opj(DATA_DIR, args.split_type,
                           args.split_name, 'random_train_cv%d.csv' % args.fold)
    ...
    valid_split_file = opj(DATA_DIR, args.split_type,
                           args.split_name, 'random_valid_cv%d.csv' % args.fold)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        iter, train_loss, train_dice = train(
            train_loader, model, ema_model, criterion, optimizer, epoch, args, lr=lr)

        with torch.no_grad():
            if args.ema:
                valid_loss, valid_dice = validate(
                    valid_loader, ema_model, criterion, epoch)
            else:
                valid_loss, valid_dice = validate(
    ...
    for iter, iter_data in enumerate(train_loader, 0):
        ...
        if (iter + 1) % print_freq == 0 or iter == 0 or (iter + 1) == num_its:
            print('\r%5.1f   %5d    %0.6f   |  %0.4f  %0.4f  | ... ' %
                  (epoch - 1 + (iter + 1) / num_its,
                   iter + 1, lr, losses.avg, dices.avg),
                  end='', flush=True)
...
def accumulate(model1, model2, decay=0.99):
    ...
    with torch.no_grad():
        for k in par1.keys():
            par1[k].data.copy_(par1[k].data * decay +
                               par2[k].data * (1 - decay))
